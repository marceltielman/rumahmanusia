...
// .wrangler/tmp/pages-Pqvb6I/functionsWorker-0.6308540802303807.mjs
...
var MAX_FIELD = 2e3;
var MIN_FILL_MS = 3e3;
var FIELDS = ["name", "org", "email", "topic"];
async function onRequestPost(context2) {
  const { request, env: env2 } = context2;
  const wantsJson = (request.headers.get("accept") ?? "").includes("application/json");
  const reply = /* @__PURE__ */ __name2((status, ok, message) => wantsJson ? Response.json({ ok, message }, { status }) : htmlResponse(status, ok, message), "reply");
  try {
    ...
    const to = env2.ENQUIRY_TO;
    const from = env2.ENQUIRY_FROM;
    ...
    const sent = await send2({ key, to, from, values, request });
    if (!sent.ok) {
    ...
    console.error("enquiry: unhandled", error3);
...
__name2(readForm, "readForm");
async function send2(args) {
...
__name(send2, "send2");
__name2(send2, "send");
function htmlResponse(status, ok, message) {
  const escape = /* @__PURE__ */ __name2((s) => s.replace(/[&<>"]/g, (c) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;" })[c]), "escape");
  return new Response(
    `<!DOCTYPE html>
...
// .wrangler/tmp/bundle-BZOypS/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-BZOypS/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.6308540802303807.js.map
